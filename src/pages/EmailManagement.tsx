...
import { Send, Upload, Plus, Menu, X, Paperclip, Mic, ChevronRight } from 'lucide-react';
...
const EmailManagement = () => {
  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-black' : 'bg-white'} relative overflow-hidden flex`}>
      ...
      {/* Sidebar */}
      <div className={`${sidebarOpen ? 'w-80' : 'w-0'} transition-all duration-300 ${theme === 'dark' ? 'bg-black/40' : 'bg-white/40'} backdrop-blur-md border-r ${theme === 'dark' ? 'border-white/10' : 'border-gray-200/30'} flex flex-col relative z-10 overflow-hidden`}>
        ...
        <div className={`flex items-center justify-between p-4 border-b ${theme === 'dark' ? 'border-white/10' : 'border-gray-200/30'} ${theme === 'dark' ? 'bg-black/20' : 'bg-white/20'} backdrop-blur-md`}>
          <div className="flex items-center gap-4">
            ...
            <h1 className={`text-xl font-semibold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
              {getCurrentChat()?.title || 'Email Management'}
            </h1>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="p-6 border-t border-inherit">
            <div className="max-w-3xl mx-auto">
              <div className={`relative ${theme === 'dark' ? 'bg-white/5' : 'bg-gray-50'} rounded-2xl border ${theme === 'dark' ? 'border-white/10' : 'border-gray-200'} focus-within:border-purple-500/50 transition-colors`}>
                <Textarea
                  value={message}
                  onChange={(e) => setMessage(e.target.value)}
                  placeholder="Ask anything..."
                  className={`min-h-[60px] max-h-[200px] resize-none border-none ${theme === 'dark' ? 'bg-transparent text-white placeholder-gray-400' : 'bg-transparent text-gray-800 placeholder-gray-500'} focus:ring-0 focus-visible:ring-0 pr-20`}
                  onKeyPress={(e) => {
